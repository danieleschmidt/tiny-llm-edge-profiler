...
class ResourcePool(Generic[T]):
    ...
    def acquire(self, timeout: Optional[float] = None, **creation_kwargs) -> ContextManager[T]:
        """
    ...
    def _acquire_resource(self, timeout: Optional[float] = None, **creation_kwargs) -> T:
    ...
    def _perform_maintenance(self) -> None:
        ...
        with self._lock:
            ...
            for resource_id, resource_info in self._resources.items():
                ...
                if (resource_info.is_expired(self.max_age_seconds) or 
                    resource_info.is_idle(self.max_idle_seconds)):
    ...
    class _ResourceContext:
        ...
        def __init__(self, pool: 'ResourcePool', timeout: Optional[float], creation_kwargs: Dict[str, Any]):
            self.pool = pool
        ...
        def __enter__(self) -> T:
            self.resource = self.pool._acquire_resource(self.timeout, **self.creation_kwargs)
...
class DeviceConnectionFactory(ResourceFactory):
    ...
    def create_resource(self, **kwargs) -> Any:
        ...
        if not profiler.connect():
            raise ResourceError(
                operation="create_device_connection",
                required_mb=0,
                available_mb=0
            ).with_message(f"Failed to connect to {self.platform} device at {self.device_path}")
...
def create_model_pool(
    ...
    **kwargs
    ...
    """Create a model cache pool."""
    return _global_pool_manager.create_model_pool(
        pool_name, model_path, **kwargs
    )
