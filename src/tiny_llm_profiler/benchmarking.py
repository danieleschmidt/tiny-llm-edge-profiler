    ...
    async def execute_operation(self, profiler: EdgeProfiler) -> Tuple[float, Any]:
        ...
        try:
            ...
            profile_data = {
                ...
                "memory_usage_mb": sequence["model_size_mb"]
...
class ConcurrencyWorkload(BenchmarkWorkload):
    ...
    def validate_result(self, result: Any) -> bool:
        ...
        return (
            ...
            and "local_operations" in result
            and result["local_operations"] > 0
        )
...
class BenchmarkRunner:
    ...
    async def run_benchmark(self, config: BenchmarkConfiguration) -> BenchmarkResult:
        ...
        try:
            ...
            # Validate results
            result.validation_passed, result.validation_details = (
            ...
            logger.info(
                f"Benchmark completed: {config.name} - {result.metrics.operations_per_second:.2f} ops/sec"
            )
    ...
    async def _run_warmup(
        self, workload: BenchmarkWorkload, profiler: EdgeProfiler, duration: float
    ):
        """Run warmup phase."""
    ...
    async def _run_benchmark_phase(
        ...
        logger.debug(
            f"Running benchmark phase: {config.iterations} iterations, {config.concurrency} concurrency"
        )
    ...
    async def _run_iteration_based(
        ...
        # Create all tasks
        tasks = [
            asyncio.create_task(execute_with_semaphore()) for _ in range(iterations)
        ]
    ...
    def _calculate_metrics(
        ...
        if latency_measurements:
            ...
            metrics.execution_duration_seconds = (
                ...
                if config.duration_seconds > 0
                else sum(latency_measurements) / 1000
            )
            metrics.operations_per_second = metrics.operations_completed / max(
                metrics.execution_duration_seconds, 0.001
            )
            ...
            metrics.latency_std_dev = (
                statistics.stdev(latency_measurements)
                if len(latency_measurements) > 1
                else 0.0
            )
            metrics.latency_variance = (
                statistics.variance(latency_measurements)
                if len(latency_measurements) > 1
                else 0.0
            )
            ...
            if metrics.average_latency_ms > 0:
                metrics.coefficient_of_variation = (
                    metrics.latency_std_dev / metrics.average_latency_ms
                )
        ...
        if metrics.operations_per_second > 0:
            ...
            metrics.bytes_processed = int(
                metrics.operations_completed * avg_data_size_bytes
            )
            metrics.bytes_per_second = metrics.bytes_processed / max(
                metrics.execution_duration_seconds, 0.001
            )
    ...
    def _validate_benchmark_result(
        self, result: BenchmarkResult, config: BenchmarkConfiguration
...
class BenchmarkComparator:
    ...
    def compare_results(
        ...
        comparison_func = self.comparison_methods.get(
            method, self._statistical_comparison
        )
    ...
    def _statistical_comparison(
        self, baseline: BenchmarkResult, comparison: BenchmarkResult
    ) -> BenchmarkComparison:
        ...
        result.ops_per_second_change_percent = self._calculate_percentage_change(
            baseline.metrics.operations_per_second,
            comparison.metrics.operations_per_second,
        )
        ...
        performance_score = (
            ...
            * latency_weight  # Lower latency is better
    ...
    def _percentage_comparison(
        ...
        avg_change = (
            result.ops_per_second_change_percent
            + (-result.latency_change_percent)
            + (-result.memory_change_percent)
            + (-result.cpu_change_percent)
        ) / 4
    ...
    def _absolute_comparison(
        ...
        significant_latency_change = (
            abs(
                comparison.metrics.average_latency_ms
                - baseline.metrics.average_latency_ms
            )
            > 50
        )
        ...
        result.performance_improvement = (
            comparison.metrics.operations_per_second
            > baseline.metrics.operations_per_second
            and comparison.metrics.average_latency_ms
            <= baseline.metrics.average_latency_ms
        )

        result.regression_detected = (
            comparison.metrics.operations_per_second
            < baseline.metrics.operations_per_second * 0.9
            or comparison.metrics.average_latency_ms
            > baseline.metrics.average_latency_ms * 1.2
        )

        result.significance_level = (
            "high" if (significant_ops_change or significant_latency_change) else "low"
        )
...
class BenchmarkSuite:
    ...
    def __init__(self, results_dir: Optional[Path] = None):
        self.results_dir = (
            results_dir or Path.home() / ".cache" / "tiny_llm_profiler" / "benchmarks"
        )
    ...
    async def run_benchmark_suite(
        self, configurations: List[BenchmarkConfiguration], save_results: bool = True
        ...
        for config in configurations:
            try:
                ...
                logger.info(
                    f"Completed benchmark: {config.name} - Status: {result.status.value}"
                )
    ...
    def generate_benchmark_report(
        self, include_historical: bool = True, include_comparisons: bool = True
    ) -> Dict[str, Any]:
            ...
            for filepath in self.results_dir.glob(pattern):
                try:
                    ...
                    if result_data["end_time"]:
                        result.end_time = datetime.fromisoformat(
    ...
    def get_benchmark_trends(self, benchmark_id: str, days: int = 30) -> Dict[str, Any]:
        ...
        with self._lock:
            ...
            recent_results = [
                r
                for r in self.results[benchmark_id]
            ...
            trends = {
                ...
                "statistics": {
                    "ops_per_second": {
                        ...
                        "std": (
                            statistics.stdev(ops_per_second)
                            if len(ops_per_second) > 1
                            else 0.0
                        ),
                    },
                    "latency_ms": {
...
def compare_benchmark_performance(
    benchmark_id: str, method: ComparisonMethod = ComparisonMethod.STATISTICAL
) -> Optional[BenchmarkComparison]:
