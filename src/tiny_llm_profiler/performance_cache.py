...
class PerformanceCache:
    ...
    def create_key(self, platform: str, model_size_mb: float, quantization: str,
                   prompts: List[str], config: Dict[str, Any] = None) -> CacheKey:
    ...
    def load_from_disk(self, file_path: Path) -> bool:
        ...
        try:
            ...
            with self.lock:
                ...
                self.access_order = [
                    key for key in cache_data.get("access_order", [])
...
class OptimizedProfiler:
    ...
    def batch_profile_with_cache(
        self,
        profile_requests: List[Tuple[str, float, str, List[str], Dict[str, Any]]]
    ) -> List[Dict[str, Any]]:
    ...
    def get_performance_stats(self) -> Dict[str, Any]:
        ...
        avg_profile_time = sum(self.profile_times) / len(self.profile_times) if self.profile_times else 0
