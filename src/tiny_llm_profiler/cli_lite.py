...
try:
    from .core_lite import SimpleProfiler, SimplePlatformManager, run_basic_benchmark, analyze_benchmark_results
except ImportError:
...
def cmd_platforms():
    ...
    headers = ["Platform", "Name", "RAM (KB)", "Flash (KB)", "Max Freq (MHz)", "Features"]
    rows = []
...
def cmd_benchmark(platforms: List[str] = None, output: str = None):
    ...
    # Display results
    headers = ["Platform", "Tokens/sec", "Latency (ms)", "Memory (KB)", "Power (mW)", "Status"]
    rows = []
    ...
    print_table(headers, rows, f"Benchmark Results (completed in {benchmark_time:.1f}s)")
    ...
    if "error" not in analysis:
        ...
        print(f"  • Average performance: {stats['avg_tokens_per_second']:.1f} tokens/sec")
