...
class ProfileResults:
    ...
    def add_latency_profile(self, profile: LatencyProfile) -> None:
        ...
        if not self.throughput_profile:
            self.throughput_profile = ThroughputProfile(
                tokens_per_second=profile.tokens_per_second, requests_per_second=0.0
            )
        else:
    ...
    def calculate_efficiency_score(self) -> float:
        ...
        if self.latency_profile:
            ...
            latency_score = min(
                100, (self.latency_profile.tokens_per_second / 20.0) * 100
            )
        ...
        if self.power_profile:
            ...
            power_score = max(
                0, 100 - (self.power_profile.active_power_mw / 200.0) * 50
            )
    ...
    def compare_with(self, other: "ProfileResults") -> Dict[str, float]:
        ...
        if self.latency_profile and other.latency_profile:
            comparison["tokens_per_second_ratio"] = (
                self.latency_profile.tokens_per_second
                / other.latency_profile.tokens_per_second
            )
            comparison["latency_improvement"] = (
                other.latency_profile.total_latency_ms
                / self.latency_profile.total_latency_ms
            )
        ...
        if self.power_profile and other.power_profile:
            ...
            comparison["energy_efficiency_ratio"] = (
                other.power_profile.energy_per_token_mj
                / self.power_profile.energy_per_token_mj
            )
    ...
    def get_recommendations(self) -> List[str]:
        ...
        if self.latency_profile:
            if self.latency_profile.tokens_per_second < 5:
                recommendations.append(
                    "Consider more aggressive quantization (2-bit) to improve latency"
                )

            if self.latency_profile.first_token_latency_ms > 200:
                recommendations.append(
                    "First token latency is high - optimize model loading or use model caching"
                )
        ...
        if self.memory_profile:
            ...
            if memory_usage_mb > 1.5:
                recommendations.append(
                    "High memory usage - consider smaller context length or 2-bit quantization"
                )

            if self.memory_profile.fragmentation_percent > 20:
                recommendations.append(
                    "High memory fragmentation - consider memory pool optimization"
                )
        ...
        if self.power_profile:
            if self.power_profile.active_power_mw > 150:
                recommendations.append(
                    "High power consumption - consider CPU frequency scaling or sleep modes"
                )

            if self.power_profile.energy_per_token_mj > 5:
                recommendations.append(
                    "High energy per token - optimize inference loop or reduce model complexity"
                )
        ...
        if self.platform in ["esp32", "esp32s3"] and self.memory_profile:
            if self.memory_profile.peak_memory_kb > 400:
                recommendations.append(
                    "ESP32: Enable PSRAM usage to handle larger models"
                )

        elif self.platform.startswith("stm32") and self.memory_profile:
            if self.memory_profile.peak_memory_kb > 150:
                recommendations.append(
                    "STM32: Model too large for typical SRAM - consider flash execution or smaller model"
                )
    ...
    def __repr__(self) -> str:
        ...
        if "latency" in summary:
            lat = summary["latency"]
            lines.append(
                f"  Latency: {lat['tokens_per_second']:.1f} tok/s, {lat['first_token_ms']:.0f}ms first token"
            )
        ...
        if "power" in summary:
            pwr = summary["power"]
            lines.append(
                f"  Power: {pwr['active_power_mw']:.0f}mW, {pwr['energy_per_token_mj']:.1f}mJ/token"
            )
