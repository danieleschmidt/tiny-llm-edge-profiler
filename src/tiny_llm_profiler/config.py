...
class SecurityConfig:
    ...
    allowed_model_extensions: List[str] = field(default_factory=lambda: ['.gguf', '.ggml', '.bin', '.safetensors'])
...
class AppConfig(BaseModel):
    ...
    device_defaults: Dict[str, Any] = Field(default_factory=lambda: {
        "baudrate": 921600,
        "timeout_seconds": 30,
    ...
    model_repository: Dict[str, Any] = Field(default_factory=lambda: {
...
class ConfigManager:
        ...
        try:
            ...
            with open(safe_path, 'r') as f:
                if safe_path.suffix.lower() in {'.yaml', '.yml'}:
                    ...
                    raise ConfigurationError(f"Unsupported config file format: {safe_path.suffix}")
    ...
    def _load_from_environment(self) -> Dict[str, Any]:
        ...
        env_mappings = {
            ...
            # Security
            'TINY_LLM_SECURITY_VALIDATION': ('security', 'enable_input_validation', bool),
            ...
            if env_var in os.environ:
                ...
                if type_info:
                    ...
                    try:
                        ...
                        logger.warning(f"Invalid value for {env_var}: {value} (expected {value_type.__name__})")
                        continue
