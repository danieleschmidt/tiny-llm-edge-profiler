...
class DeviceConnectionError(DeviceError):
    ...
    def __init__(self, device_path: str, platform: str, original_error: Optional[Exception] = None):
...
class DeviceTimeoutError(DeviceError):
    ...
    def __init__(self, operation: str, timeout_seconds: float):
        message = f"Device operation '{operation}' timed out after {timeout_seconds} seconds"
        details = {
            "operation": operation,
...
class ModelValidationError(ModelError):
    ...
    def __init__(self, model_name: str, validation_issues: List[str]):
        message = f"Model '{model_name}' failed validation: {'; '.join(validation_issues)}"
        details = {
...
class ModelFormatError(ModelError):
    ...
    def __init__(self, model_path: str, expected_format: Optional[str] = None, detected_format: Optional[str] = None):
...
class ProfilingTimeoutError(ProfilingError):
    ...
    def __init__(self, operation: str, timeout_seconds: float, partial_results: Optional[Dict[str, Any]] = None):
...
class InsufficientResultsError(AnalysisError):
    ...
    def __init__(self, analysis_type: str, available_results: int, required_results: int):
...
def validate_input(value: Any, validation_rules: Dict[str, Any], parameter_name: str) -> Any:
    """
    ...
    if isinstance(value, (int, float)):
        if 'min_value' in validation_rules and value < validation_rules['min_value']:
            raise InputValidationError(
                parameter_name, 
                value, 
                f"must be >= {validation_rules['min_value']}"
            )
        ...
        if 'max_value' in validation_rules and value > validation_rules['max_value']:
            raise InputValidationError(
                parameter_name, 
                value, 
                f"must be <= {validation_rules['max_value']}"
            )
    ...
    if isinstance(value, str):
        ...
        if validation_rules.get('safe_path', False):
            import os
            if '..' in value or os.path.isabs(value) and not validation_rules.get('allow_absolute', False):
                raise UnsafePathError(value, "contains potentially unsafe path components")
    
    # List validation
    if isinstance(value, list):
        if 'min_items' in validation_rules and len(value) < validation_rules['min_items']:
            raise InputValidationError(
                parameter_name, 
                value, 
        ...
        if 'max_items' in validation_rules and len(value) > validation_rules['max_items']:
            raise InputValidationError(
                parameter_name, 
                value, 
...
def safe_file_path(path: str, base_directory: Optional[str] = None) -> str:
    ...
    if base_directory:
        ...
        if not abs_path.startswith(base_abs):
            raise UnsafePathError(path, f"path outside allowed directory {base_directory}")
