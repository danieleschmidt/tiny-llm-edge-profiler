...
class PlatformManager:
    ...
    def _get_builtin_configs(self) -> Dict[str, PlatformConfig]:
        ...
        return {
            ...
            "stm32f4": PlatformConfig(
                ...
                memory=MemoryConfiguration(ram_kb=192, flash_kb=2048, cache_kb=8),
                capabilities=PlatformCapabilities(
                    has_fpu=True, has_dsp=True, max_cpu_freq_mhz=168, min_cpu_freq_mhz=1
                ),
            ...
            "stm32f7": PlatformConfig(
                ...
                memory=MemoryConfiguration(
                    ram_kb=512, flash_kb=2048, cache_kb=32  # I-cache + D-cache
                ),
                capabilities=PlatformCapabilities(
                    has_fpu=True, has_dsp=True, max_cpu_freq_mhz=216, min_cpu_freq_mhz=1
                ),
                toolchain="arm-none-eabi-gcc",
                flash_command="st-flash",
                debug_interface="swd",
            ...
            "rp2040": PlatformConfig(
                ...
                capabilities=PlatformCapabilities(
                    supports_dual_core=True, max_cpu_freq_mhz=133, min_cpu_freq_mhz=1
                ),
                toolchain="arm-none-eabi-gcc",
            ...
            "nrf52840": PlatformConfig(
                ...
                architecture=Architecture.ARM_CORTEX_M4,
                memory=MemoryConfiguration(ram_kb=256, flash_kb=1024),
                capabilities=PlatformCapabilities(
                    has_fpu=True,
                    supports_bluetooth=True,
                    max_cpu_freq_mhz=64,
            ...
            "bl602": PlatformConfig(
                ...
                memory=MemoryConfiguration(ram_kb=276, flash_kb=2048),
                capabilities=PlatformCapabilities(
                    has_fpu=True,
                    supports_wifi=True,
                    supports_bluetooth=True,
            ...
            "rpi_zero": PlatformConfig(
                ...
                capabilities=PlatformCapabilities(
                    has_fpu=True, max_cpu_freq_mhz=1000, min_cpu_freq_mhz=700
                ),
    ...
    def get_optimization_recommendations(self, model_size_mb: float) -> Dict[str, Any]:
        ...
        if self.config.architecture == Architecture.ARM_CORTEX_M0_PLUS:
            ...
            recommendations["context_length"] = min(
                recommendations["context_length"], 512
            )
    ...
    def validate_model_compatibility(
        self, model_size_mb: float, quantization: str
    ) -> tuple[bool, List[str]]:
        """
        ...
        if model_size_mb > available_ram_mb:
            if self.config.memory.external_ram_kb > 0:
                total_available_mb = (
                    ...
                    + self.config.memory.external_ram_kb
                ) / 1024
                if model_size_mb > total_available_mb:
                    issues.append(
                        f"Model size ({model_size_mb:.1f}MB) exceeds total available memory ({total_available_mb:.1f}MB)"
                    )
            else:
                issues.append(
        ...
        if quantization not in self.config.supported_quantizations:
            issues.append(
                f"Quantization '{quantization}' not supported on {self.config.display_name}"
            )

        # Platform-specific checks
        if self.config.architecture in [
            Architecture.ARM_CORTEX_M0,
            Architecture.ARM_CORTEX_M0_PLUS,
        ]:
            if model_size_mb > 1.0:  # Very conservative for M0/M0+
                issues.append(
                    "Models larger than 1MB may be too slow on Cortex-M0/M0+ platforms"
                )
    ...
    def get_flash_command(self, firmware_path: str, device_port: str = None) -> str:
        ...
        return command_templates.get(
            self.config.flash_command, f"{self.config.flash_command} {firmware_path}"
        )
