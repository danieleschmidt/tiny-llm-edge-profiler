...
class TaskResult(Generic[T]):
    ...
    def mark_completed(
        self, result: Optional[T] = None, error: Optional[Exception] = None
    ):
        ...
        with self._lock:
            ...
            for i in range(self.max_threads):
                worker = threading.Thread(
                    target=self._worker_loop, name=f"ProfilerWorker-{i}", daemon=True
                )
                worker.start()
    ...
    def submit_task(self, task: ProfilingTask) -> str:
        ...
        with self._lock:
            ...
            try:
                ...
                logger.debug(
                    f"Submitted task: {task.task_id} (priority: {task.priority})"
                )
                return task.task_id

            except queue.Full:
                raise TinyLLMProfilerError(
                    f"Task queue full (max size: {self.queue_size})", "QUEUE_FULL"
                )
...
class BatchProfiler:
    ...
    def profile_multiple_models(
        ...
        for i in range(0, len(tasks), batch_size):
            ...
            if i + batch_size < len(tasks):  # Not the last batch
                self.concurrent_profiler.wait_for_completion(
                    batch_task_ids, timeout=300  # 5 minutes per batch
                )

        # Wait for all tasks to complete
        results = self.concurrent_profiler.wait_for_completion(
            submitted_tasks, timeout=600  # 10 minutes total
        )
        ...
        self.logger.info(
            f"Batch profiling completed: {len(organized_results)} successful results"
        )
...
class AsyncProfiler:
    ...
    async def profile_async(
        self, tasks: List[ProfilingTask], max_concurrent: int = 10
    ) -> Dict[str, TaskResult]:
        """
            ...
            async with semaphore:
                ...
                try:
                    ...
                    result = await loop.run_in_executor(
                        None, self._run_sync_profiling, task
                    )
