...
class HealthChecker:
    ...
    def get_overall_health(self) -> HealthStatus:
        ...
        healthy_checks = sum(
            1 for status in self.last_check_results.values() if status.healthy
        )
    ...
    def get_metrics_summary(self, duration_minutes: int = 10) -> Dict[str, Any]:
        ...
        summary = {
            ...
            "disk": {"free_gb": recent_metrics[-1].disk_free_gb},
        }
    ...
    def _check_system_resources(self) -> HealthStatus:
        ...
        try:
            ...
            if metrics.memory_available_mb < 100:  # Less than 100MB available
                issues.append(
                    f"Low available memory: {metrics.memory_available_mb:.1f}MB"
                )
    ...
    def _check_disk_space(self) -> HealthStatus:
        ...
        try:
            checks = {"root": "/", "tmp": "/tmp", "home": str(Path.home())}
            ...
            message = (
                "Disk space OK"
                if healthy
                else f"Disk space issues: {'; '.join(issues)}"
            )
    ...
    def _check_python_environment(self) -> HealthStatus:
        ...
        try:
            ...
            if version < (3, 8):
                issues.append(
                    f"Python version {version.major}.{version.minor} is too old (minimum: 3.8)"
                )
            ...
            message = (
                ...
                if healthy
                else f"Environment issues: {'; '.join(issues)}"
            )
    ...
    def _check_security_environment(self) -> HealthStatus:
        ...
        try:
            ...
            healthy = (
                security_results["secure"] and len(security_results["errors"]) == 0
            )

            issues = security_results["errors"] + security_results["warnings"]
            message = (
                "Security environment OK"
                if healthy
                else f"Security issues: {'; '.join(issues[:3])}"
            )
    ...
    def _generate_recommendations(
        self, check_results: Dict[str, HealthStatus], metrics_summary: Dict[str, Any]
    ) -> List[str]:
        ...
        if (
            "memory" in metrics_summary
            and metrics_summary["memory"]["average_percent"] > 80
        ):
            recommendations.append(
                "Monitor memory usage - consider optimizing memory allocation or adding more RAM"
            )
        ...
        for name, status in check_results.items():
            if not status.healthy:
                if name == "dependencies":
                    ...
                    recommendations.append(
                        "Consider upgrading Python version or using a virtual environment"
                    )
                elif name == "security_environment":
                    recommendations.append(
                        "Review security warnings and apply recommended fixes"
                    )
