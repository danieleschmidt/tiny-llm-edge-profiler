...
class InMemoryCache:
    ...
    def _ensure_space(self, needed_bytes: int) -> None:
        ...
        # Check memory limit
        while (
            self._total_size_bytes + needed_bytes > self.max_memory_bytes
...
class PersistentCache:
        ...
        with self._lock:
            try:
                ...
                if len(data) > self.max_size_bytes:
                    logger.warning(
                        f"Value too large for persistent cache: {len(data)} bytes"
                    )
                    return
    ...
    def _ensure_space(self, needed_bytes: int) -> None:
        ...
        entries = [
            (key_hash, entry_info) for key_hash, entry_info in self._index.items()
        ]
    ...
    def __init__(
        ...
        if persistent_cache_dir:
            self.persistent_cache = PersistentCache(
                cache_dir=persistent_cache_dir, max_size_gb=persistent_cache_gb
            )
