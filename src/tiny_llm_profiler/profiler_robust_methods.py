...
def _profile_latency_robust(
    ...
    def measure_latency_sample(prompt: str, iteration: int) -> dict:
        ...
        try:
            ...
            return {
                ...
                "tokens_per_second": total_tokens / (total_latency / 1000.0) if total_latency > 0 else 0.0
            }
            
        except Exception as e:
            self.logger.warning(f"Latency measurement failed for iteration {iteration}: {e}")
            # Log metric for monitoring
            self.perf_logger.log_metric("latency_measurement_failure", 1, "count", 
    ...
    try:
        ...
        # Measurement runs
        self.logger.info(f"Running latency measurements: {len(test_prompts)} prompts x {config.measurement_iterations} iterations")
        ...
        for prompt_idx, prompt in enumerate(test_prompts):
            ...
            for iteration in range(config.measurement_iterations):
                try:
                    ...
                    with managed_timeout(config.operation_timeout, f"latency_measurement_{prompt_idx}_{iteration}"):
                    ...
                    if not config.graceful_degradation or failed_measurements > successful_measurements:
            ...
            if prompt_measurements:
                ...
                first_token_latencies.extend([m["first_token_latency_ms"] for m in prompt_measurements])
                inter_token_latencies.extend([m["inter_token_latency_ms"] for m in prompt_measurements if m["inter_token_latency_ms"] > 0])
                tokens_per_second_samples.extend([m["tokens_per_second"] for m in prompt_measurements])
        
        # Check if we have sufficient data
        min_required_samples = max(3, config.measurement_iterations // 2)  # At least 3 or half of intended measurements
        ...
        mean_first_token = np.mean(first_token_latencies) if first_token_latencies else 0.0
        mean_inter_token = np.mean(inter_token_latencies) if inter_token_latencies else 0.0
        mean_tokens_per_sec = np.mean(tokens_per_second_samples) if tokens_per_second_samples else 0.0
        ...
        return LatencyProfile(
            ...
            success_rate=successful_measurements / (successful_measurements + failed_measurements) if (successful_measurements + failed_measurements) > 0 else 0.0
        )
        ...
        if config.graceful_degradation and latencies:
            self.logger.info("Returning partial latency results due to graceful degradation")
            return LatencyProfile(
                first_token_latency_ms=np.mean(first_token_latencies) if first_token_latencies else 0.0,
                inter_token_latency_ms=np.mean(inter_token_latencies) if inter_token_latencies else 0.0,
                total_latency_ms=np.mean(latencies),
                tokens_per_second=np.mean(tokens_per_second_samples) if tokens_per_second_samples else 0.0,
    ...
    try:
        ...
        for prompt_idx, prompt in enumerate(memory_test_prompts):
            try:
                ...
                with managed_timeout(config.operation_timeout, f"memory_inference_{prompt_idx}"):
                ...
                self.logger.warning(f"Memory measurement failed for prompt {prompt_idx}: {e}")
                if not config.graceful_degradation:
                    raise
        ...
        return MemoryProfile(
            ...
            success_rate=successful_measurements / len(memory_test_prompts) if memory_test_prompts else 0.0
        )
...
def _profile_memory_remote_robust(
    ...
    try:
        ...
        for prompt_idx, prompt in enumerate(memory_test_prompts):
            try:
                # Run inference
                with managed_timeout(config.operation_timeout, f"remote_memory_inference_{prompt_idx}"):
                    self._run_inference_robust(model, prompt)
                ...
                self.logger.warning(f"Remote memory measurement failed for prompt {prompt_idx}: {e}")
                if not config.graceful_degradation:
                    raise
...
def _profile_power_robust(
    ...
    try:
        ...
        total_energy_mj = (active_power_mw * estimated_inference_time_sec) / 1000.0  # Convert to mJ
        energy_per_token_mj = total_energy_mj / estimated_tokens_total if estimated_tokens_total > 0 else 0.0
        
        self.logger.info("Power profiling completed with simulated values")
        self.logger.warning("Power profiling is using simulated values - integrate with actual power measurement hardware for real data")
    ...
    def measure_first_token() -> float:
        ...
        if self.connection == "local":
            ...
            try:
                ...
                self.logger.warning(f"First token measurement communication failed: {e}")
                raise
