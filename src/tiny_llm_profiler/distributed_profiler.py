...
class NodeInfo:
    ...
    def can_accept_task(self) -> bool:
        ...
        return (
            self.status in [NodeStatus.ACTIVE, NodeStatus.BUSY]
...
class DistributedTask:
    ...
    def can_retry(self) -> bool:
        ...
        return (
            self.status == DistributedTaskStatus.FAILED
...
class NetworkCommunicator:
    ...
    def send_to_node(
        self, target_host: str, target_port: int, message: Dict[str, Any]
    ) -> bool:
    ...
    def broadcast_to_nodes(
        self, nodes: List[Tuple[str, int]], message: Dict[str, Any]
    ) -> int:
...
class DistributedCoordinator:
    ...
    def __init__(
        ...
        self.communicator = NetworkCommunicator(self.coordinator_id, host, port)
                    ...
                    if not assigned:
                        ...
                        with self.task_lock:
                            heapq.heappush(
                                self.task_queue,
                                (
                                    -task_to_schedule.priority + 1,
                                    task_to_schedule.task_id,
                                    task_to_schedule,
                                ),
                            )
        ...
        with self.node_lock:
            ...
            if task.preferred_nodes:
                ...
                preferred_candidates = [
                    (nid, ninfo)
                    for nid, ninfo in candidate_nodes
            ...
            if selected_node_id:
                ...
                if success:
                    logger.info(
                        f"Assigned task {task.task_id} to node {selected_node_id}"
                    )
                    return True
                else:
    ...
    def _send_task_to_node(self, task: DistributedTask, node_info: NodeInfo) -> bool:
        ...
        try:
            ...
            message = NetworkProtocol.create_message(
                NetworkProtocol.TASK_ASSIGNMENT, task_data, self.coordinator_id
            )

            # Send to node
            success = self.communicator.send_to_node(
                node_info.host, node_info.port, message
            )
    ...
    def _handle_node_registration(
        ...
        try:
            ...
            response = NetworkProtocol.create_message(
                "registration_response", {"success": success}, self.coordinator_id
            )
                ...
                if task_id in self.active_tasks:
                    ...
                    if data.get("success", False):
                        ...
                        if task.can_retry():
                            ...
                            # Put back in queue
                            heapq.heappush(
                                self.task_queue, (-task.priority, task.task_id, task)
                            )
                    ...
                    if task.status != DistributedTaskStatus.RETRYING:
                        ...
                        if task.get_execution_duration():
                            ...
                            ] += task.get_execution_duration()

                    # Update node info
                    with self.node_lock:
                        if sender_id in self.nodes:
                            node = self.nodes[sender_id]
                            node.current_task_count = max(
                                0, node.current_task_count - 1
                            )
        ...
        with self.node_lock:
            ...
            with self.task_lock:
                ...
                for task in tasks_to_reassign:
                    ...
                    logger.info(
                        f"Reassigned task {task.task_id} from failed node {node_id}"
                    )
    ...
    def _notify_nodes_shutdown(self):
        ...
        message = NetworkProtocol.create_message(
            "coordinator_shutdown", {}, self.coordinator_id
        )
...
def get_distributed_coordinator(
    coordinator_id: Optional[str] = None, host: str = "localhost", port: int = 8000
...
def submit_distributed_profiling_task(
    ...
    return coordinator.submit_distributed_task(
        profiling_task, priority=priority, preferred_nodes=preferred_nodes
    )
