...
class NetworkAnalyzer:
    ...
    async def measure_latency(self, from_region: RegionInfo, to_region: RegionInfo) -> NetworkLatencyInfo:
        ...
        try:
            ...
            logger.error(f"Failed to measure latency from {from_region.region_id} to {to_region.region_id}: {e}")
        ...
        try:
            ...
            with concurrent.futures.ThreadPoolExecutor() as executor:
                result = await loop.run_in_executor(
                    executor, 
                    lambda: ping3.ping(hostname, timeout=self.timeout) if PING3_AVAILABLE else 0.1
                )
                return result
        except Exception:
            return None
    ...
    def calculate_geographic_distance(self, region1: RegionInfo, region2: RegionInfo) -> float:
    ...
    def _calculate_target_weights(
        ...
        for target_id, target_region in regions.items():
            ...
            if strategy == OptimizationStrategy.LATENCY_FIRST:
                ...
                reliability_score = (target_region.uptime_percent / 100) * target_region.health_score() / 100
                weight = reliability_score * (1.0 - network_info.packet_loss_percent / 100)
    ...
    def get_routing_recommendations(
        ...
        for source_region, targets in routing.items():
            ...
            high_traffic_targets = [
                target for target, weight in targets.items()
...
class RegionManager:
    ...
    def register_region(self, region_info: RegionInfo):
        ...
        with self._lock:
            ...
            logger.info(f"Registered region: {region_info.region_id} ({region_info.name})")
    
    def update_region_status(self, region_id: str, status: RegionStatus, metadata: Optional[Dict] = None):
        ...
        for i, source_region in enumerate(region_list):
            for target_region in region_list[i:]:
                if source_region.region_id != target_region.region_id:
                    ...
                    tasks.append(
                        self.network_analyzer.measure_latency(source_region, target_region)
                    )
                    tasks.append(
                        self.network_analyzer.measure_latency(target_region, source_region)
                    )
            ...
            for region_id, region in self.regions.items():
                ...
                if region.health_score() < 50:
                    recommendations.append({
                        ...
                        "actions": ["Check region status", "Investigate performance issues", "Consider failover"]
                    })
                ...
                if utilization > 90:
                    recommendations.append({
                        ...
                        "actions": ["Scale up resources", "Implement load balancing", "Add capacity"]
                    })
                elif utilization < 10:
                    recommendations.append({
                        ...
                        "actions": ["Consider downsizing", "Redirect traffic", "Cost optimization"]
                    })
                
                # Check connectivity
                if len(region.connected_regions) < len(self.regions) - 1:
                    missing_connections = set(self.regions.keys()) - region.connected_regions - {region_id}
                    if missing_connections:
                        recommendations.append({
                            ...
                            "actions": ["Check network configuration", "Verify endpoints", "Test connectivity"]
                        })
        
        return recommendations
    
    def get_regions(self, status: Optional[RegionStatus] = None) -> Dict[str, RegionInfo]:
...
class GlobalOptimizer:
    ...
    async def optimize_deployment(
        ...
        cost_estimate = self._estimate_deployment_cost(service_placements, resource_allocations, regions)
        latency_improvement = self._estimate_latency_improvement(network_matrix, traffic_routing)
        ...
        record_performance_metric(
            "global_optimization_latency_improvement", latency_improvement, tags={"strategy", strategy.value}
        )
        
        logger.info(f"Global optimization completed: {strategy.value} strategy, cost=${cost_estimate:.2f}, latency improvement={latency_improvement:.1f}%")
    ...
    def _optimize_service_placement(
        ...
        for region_id, region in regions.items():
            if strategy == OptimizationStrategy.COST_FIRST:
                ...
                avg_latency = np.mean([
                    info.avg_latency_ms for key, info in network_matrix.items()
                    if key[0] == region_id
                ]) if any(key[0] == region_id for key in network_matrix.keys()) else 1000
        ...
        for service in services:
            ...
            for region_id, score in available_regions:
                ...
                logger.debug(f"Placed service {service} in region {region_id} (score: {score:.3f})")
            
            if placed_count < required_redundancy:
                logger.warning(f"Could not achieve required redundancy for service {service} ({placed_count}/{required_redundancy})")
    ...
    def _calculate_resource_allocations(
        ...
        for region_id, services in service_placements.items():
            ...
            memory_needed = total_services * base_memory_per_service * traffic_multiplier  
            ...
            allocations[region_id] = {
                "cpu": min(cpu_needed, max_cpu * 0.9),  # Don't use more than 90% capacity
    ...
    def _calculate_optimization_confidence(
        ...
        if network_matrix:
            avg_quality = np.mean([info.quality_score() for info in network_matrix.values()])
        ...
        placement_confidence = min(1.0, total_services / max(1, len(regions)))  # Services per region ratio
    ...
    def _optimization_loop(self):
        ...
        while self.running:
            try:
                ...
                active_regions = len(self.region_manager.get_regions(RegionStatus.ACTIVE))
                record_performance_metric("global_optimizer_active_regions", float(active_regions))
        ...
        while self.running:
            try:
                ...
                for region_id, region in regions.items():
                    ...
                    # Record metrics
                    record_performance_metric(
                        f"region_health_{region_id}", health_score,
                        tags={"region", region_id}
                    )
                    record_performance_metric(
                        f"region_utilization_{region_id}", utilization,
                        tags={"region", region_id}
                    )
                    
                    # Alert on issues
                    if health_score < 50:
                        logger.error(f"Region {region_id} health critical: {health_score:.1f}")
                    elif utilization > 90:
                        logger.warning(f"Region {region_id} high utilization: {utilization:.1f}%")
    ...
    def get_global_status(self) -> Dict[str, Any]:
        ...
        active_regions = len([r for r in regions.values() if r.status == RegionStatus.ACTIVE])
        avg_health = np.mean([r.health_score() for r in regions.values()]) if regions else 0.0
        avg_utilization = np.mean([r.utilization_score() for r in regions.values()]) if regions else 0.0
        ...
        return {
            ...
            "regions": {
                ...
                "status_breakdown": {
                    status.value: len([r for r in regions.values() if r.status == status])
...
async def optimize_global_deployment(
    ...
    """Optimize global deployment configuration."""
    optimizer = get_global_optimizer()
    return await optimizer.optimize_deployment(services, traffic_patterns, strategy, constraints)
