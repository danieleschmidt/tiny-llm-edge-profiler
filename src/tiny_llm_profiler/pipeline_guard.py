...
class FailurePredictor:
    ...
    def predict_failure_probability(self, metrics: PipelineMetrics) -> float:
        ...
        if len(self.metrics_history) >= 5:
            ...
            trend_slope = (
                self.trend_analyzer.coef_[0]
                if hasattr(self.trend_analyzer, "coef_")
                else 0
            )
            trend_prob = max(
                0, -trend_slope * 10
...
class BasicAutoHealer(AutoHealer):
    ...
    async def can_heal(self, failure: FailureEvent) -> bool:
        return (
            failure.recovery_attempts < 3
            and failure.failure_type != FailureType.PERSISTENT
        )
    ...
    def _select_healing_action(self, failure: FailureEvent) -> Optional[HealingAction]:
        ...
        suitable_actions = [
            ...
            if action.target_stage == failure.stage
            or action.target_stage == PipelineStage.BUILD
        ]
...
class SelfHealingPipelineGuard:
    ...
    async def _handle_failure(self, failure: FailureEvent) -> None:
        logger.warning(
            f"Failure detected in {failure.stage.value}: {failure.error_message}"
        )
        ...
        if await self.healer.can_heal(failure):
            ...
            logger.error(
                f"Cannot heal failure in {failure.stage.value} - manual intervention required"
            )
