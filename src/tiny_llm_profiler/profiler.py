...
from .exceptions import (
    DeviceConnectionError, DeviceTimeoutError, ProfilingError,
    ProfilingTimeoutError, ResourceError, InsufficientMemoryError
)
from .logging_config import get_logger, PerformanceLogger
from .reliability import (
    retry, circuit_breaker, with_timeout, managed_resource,
...
class EdgeProfiler:
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.logger.info(f"Initialized EdgeProfiler for {platform} with enhanced reliability features")
    ...
    def connect(self) -> bool:
        ...
        with self.connection_lock:
            ...
            try:
                self.logger.info(f"Attempting to connect to {self.platform} device (attempt #{self.connection_attempts})")
                
                if self.connection == "serial":
                    if not self.device:
                        raise DeviceConnectionError("no_device", self.platform, 
                                                   ValueError("Device path not specified"))
                    
                    # Pre-connection health check
                    if not self._check_device_availability():
                        raise DeviceConnectionError(self.device, self.platform,
                    ...
                    if not self._check_system_resources():
                        raise DeviceConnectionError("localhost", self.platform,
                ...
                self.perf_logger.log_device_event("connected", self.platform, self.device or "local")
    ...
    def disconnect(self):
        ...
        with self.connection_lock:
            ...
            try:
                ...
                self.perf_logger.log_device_event("disconnected", self.platform, self.device or "local")
    ...
    def _test_connection(self) -> bool:
        ...
        try:
            ...
            self.logger.debug("No response to connection test, but connection appears functional")
            return True
    ...
    def _check_system_resources(self) -> bool:
        ...
        try:
            ...
            if available_mb < 100:  # Minimum 100MB required
                self.logger.error(f"Insufficient memory: {available_mb:.1f}MB available")
                return False
            
            # Check CPU usage
            cpu_percent = psutil.cpu_percent(interval=1)
            if self.config.max_cpu_percent and cpu_percent > self.config.max_cpu_percent:
        ...
        try:
            ...
            with managed_resource(results, "profiling_results"):
                ...
                for metric in metrics:
                    try:
                        if metric == "latency" and session_config.enable_latency_profiling:
                            latency_profile = self._profile_latency_robust(model, test_prompts, session_config)
                            results.add_latency_profile(latency_profile)
                            successful_metrics.append("latency")
                        
                        elif metric == "memory" and session_config.enable_memory_profiling:
                            memory_profile = self._profile_memory_robust(model, test_prompts, session_config)
                            results.add_memory_profile(memory_profile)
                            successful_metrics.append("memory")
                            
                        elif metric == "power" and session_config.enable_power_profiling:
                            power_profile = self._profile_power_robust(model, test_prompts, session_config)
                            results.add_power_profile(power_profile)
                            successful_metrics.append("power")
                        
                    except Exception as e:
                        self.logger.error(f"Failed to profile {metric}: {e}", metric=metric)
    ...
    def _validate_profiling_inputs(
        ...
        if len(test_prompts) > max_prompts:
            raise ValueError(f"Too many test prompts: {len(test_prompts)}, max allowed: {max_prompts}")
    ...
    def _pre_profiling_health_check(self):
        ...
        if not self._check_system_resources():
            if not self.config.graceful_degradation:
                ...
                self.logger.warning("Resource constraints detected - profiling may be degraded")
        
        # Check device health
        if self.is_connected and self.device_health["status"] not in ["healthy", "connected"]:
        ...
        if cb_status["state"] != "closed":
            self.logger.warning(f"Circuit breaker is {cb_status['state']} - profiling may fail")
