...
class LoadBalancer:
    ...
    def add_resource(
        self, resource_id: str, resource: Any, capacity_weight: float = 1.0
    ) -> None:
    ...
    def _least_response_time_selection(
        ...
        return min(
            resources,
            key=lambda r: self.resource_stats[r["id"]]["average_response_time"],
        )
    ...
    def report_task_completion(
        self, resource_id: str, task_id: str, success: bool, response_time: float
    ) -> None:
        """Report task completion."""
        with self._lock:
            for resource_info in self.resources:
                if resource_info["id"] == resource_id:
                    resource_info["active_tasks"] = max(
                        0, resource_info["active_tasks"] - 1
                    )
                    if not success:
    ...
    def start(self) -> None:
        ...
        with self._lock:
            ...
            self.scaling_thread = threading.Thread(
                target=self._scaling_loop, name="AutoScaler", daemon=True
            )
        ...
        try:
            ...
            if len(self.metrics_history) > 0:
                ...
                if len(recent_metrics) >= 2:
                    ...
                    if time_span > 0:
                        task_diff = (
                            recent_metrics[-1].completed_tasks_per_minute
                            - recent_metrics[0].completed_tasks_per_minute
                        )
            ...
            if profiler_stats["completed_tasks"] > 0:
                ...
                error_rate = (
                    profiler_stats.get("tasks_failed", 0)
                    / profiler_stats["completed_tasks"]
                ) * 100
        ...
        for rule in self.scaling_rules:
            try:
                ...
                if rule.should_trigger(metric_value):
                    logger.info(
                        f"Scaling rule triggered: {rule.name} (value: {metric_value}, threshold: {rule.threshold})"
                    )
    ...
    def _scale_up(self) -> bool:
        ...
        with self._lock:
            ...
            if current_instances >= self.max_instances:
                logger.warning(
                    f"Cannot scale up: already at maximum instances ({self.max_instances})"
                )
                return False
            ...
            try:
                ...
                if new_resource:
                    ...
                    logger.info(
                        f"Scaled up: created instance {instance_id} (total: {current_instances + 1})"
                    )
                    return True
    ...
    def _scale_down(self) -> bool:
        ...
        with self._lock:
            ...
            if current_instances <= self.min_instances:
                logger.warning(
                    f"Cannot scale down: already at minimum instances ({self.min_instances})"
                )
                return False
            ...
            try:
                ...
                least_used_resource = min(
                    self.load_balancer.resources, key=lambda r: r["active_tasks"]
                )
                ...
                if least_used_resource["active_tasks"] == 0:
                    ...
                    # Clean up resource
                    self._destroy_resource_instance(
                        instance_id, least_used_resource["resource"]
                    )
                    ...
                    logger.info(
                        f"Scaled down: removed instance {instance_id} (total: {current_instances - 1})"
                    )
                    return True
                else:
        ...
        for resource_info in self.load_balancer.resources:
            try:
                ...
                if not is_healthy and resource_info["healthy"]:
                    ...
                    self.load_balancer.update_resource_health(
                        resource_info["id"], False
                    )
                ...
                logger.error(
                    f"Health check failed for resource {resource_info['id']}: {e}"
                )
        ...
        with self._lock:
            ...
            if self.metrics_history:
                recent_metrics = {
                    ...
                    "memory_usage_percent": self.metrics_history[
                        -1
