...
class INA219Sensor(PowerSensor):
    ...
    def _create_simulated_sensor(self):
        ...
        return {"voltage_base": 3.3, "current_base": 50, "noise_level": 0.1}
...
class SimulatedPowerSensor(PowerSensor):
    ...
    def read_power(self) -> PowerMeasurement:
        ...
        power_mw = (
            self.idle_power_mw
            + (self.base_power_mw - self.idle_power_mw) * load_factor * noise
        )
...
class PowerProfiler:
    ...
    def profile(
        self, duration_seconds: int, sample_rate_hz: int = 1000
    ) -> PowerProfile:
        """
    ...
    def stream_measurements(
        self, sample_rate_hz: int = 100
    ) -> Iterator[PowerMeasurement]:
    ...
    def plot_measurements(self, output_path: Path):
        ...
        fig.add_trace(
            go.Scatter(
                x=relative_times, y=power_values, mode="lines", name="Power (mW)"
            ),
            row=1,
            col=1,
        )
        ...
        fig.add_trace(
            go.Scatter(
                x=relative_times, y=voltage_values, mode="lines", name="Voltage (V)"
            ),
            row=2,
            col=1,
        )
        ...
        fig.add_trace(
            go.Scatter(
                x=relative_times, y=current_values, mode="lines", name="Current (mA)"
            ),
            row=3,
            col=1,
        )
