...
class PredictiveModel(ABC):
    @abstractmethod
    def predict_demand(
        self, current_metrics: List[ResourceMetrics], horizon_minutes: int = 15
    ...
    def predict_demand(
        ...
        for resource_type, metrics in current_data.items():
            ...
            try:
                ...
                logger.error(
                    f"Error predicting demand for {resource_type.value}: {str(e)}"
                )

        return predictions

    def _predict_resource_demand(
        self,
        resource_type: ResourceType,
        ...
        features.extend([future_time.hour, future_time.weekday(), future_time.day])
...
class MLPredictiveModel(PredictiveModel):
    ...
    def _prepare_ml_training_data(
        self, data: List[Tuple[float, float, Dict[str, Any]]]
    ) -> Tuple[List, List]:
        X, y = [], []
        ...
        for i in range(self.feature_window, len(data)):
            ...
            features.extend(
                [
                    ...
                    np.percentile(recent_values, 75)
                    - np.percentile(recent_values, 25),  # IQR
                ]
            )
    ...
    def predict_demand(
        ...
        for resource_type, metrics in current_data.items():
            ...
            try:
                ...
                logger.error(
                    f"Error in ML prediction for {resource_type.value}: {str(e)}"
                )

        return predictions

    def _predict_ml_resource_demand(
        self,
        resource_type: ResourceType,
        metrics: List[ResourceMetrics],
        horizon_minutes: int,
...
class AdaptiveScalingController:
    ...
    async def evaluate_scaling_needs(
        self, current_metrics: List[ResourceMetrics]
    ) -> List[ScalingEvent]:
        scaling_events = []
    ...
    async def _evaluate_resource_scaling(
        ...
        if self.strategy in [
            ...
            ScalingStrategy.ML_BASED,
        ]:
            predictions = self.current_model.predict_demand(
                [metric], self.prediction_horizon_minutes
            )
            predicted_utilization = predictions.get(
                metric.resource_type, current_utilization
            )
    ...
    def _determine_scaling_direction(
        self, current: float, predicted: float, rule: ScalingRule
    ) -> ScalingDirection:
    ...
    def _calculate_new_capacity(
        self, current_capacity: float, direction: ScalingDirection, rule: ScalingRule
    ) -> float:
    ...
    def _build_scaling_reason(
        ...
        if pattern_multiplier != 1.0:
            reason_parts.append(
                f"Workload pattern multiplier: {pattern_multiplier:.2f}"
            )
    ...
    async def _execute_scaling(self, event: ScalingEvent) -> Tuple[bool, float]:
        ...
        try:
            ...
            logger.info(
                f"Executing scaling: {event.resource_type.value} "
    ...
    def get_scaling_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "success_rate": self.successful_scaling_events
            / max(1, self.total_scaling_events),
            ...
            "model_status": {
                "current_model": (
                    "ML" if self.current_model == self.ml_model else "Linear"
                ),
