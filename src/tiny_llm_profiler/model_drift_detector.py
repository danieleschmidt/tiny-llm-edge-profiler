...
class StatisticalDriftDetector(DriftDetector):
    def __init__(
        self, performance_threshold: float = 0.05, distribution_threshold: float = 0.1
    ):
    ...
    def _detect_performance_drift(
        ...
        if accuracy_drift > self.performance_threshold:
            severity = self._calculate_severity(
                accuracy_drift, self.performance_threshold
            )
            alerts.append(
                DriftAlert(
    ...
    def _calculate_severity(
        self, drift_value: float, threshold: float
    ) -> DriftSeverity:
...
class ModelDriftMonitor:
    ...
    def _update_baseline_if_needed(self) -> None:
        ...
        if (
            self.last_baseline_update
            and now - self.last_baseline_update > self.baseline_update_interval
        ):
    ...
    def _calculate_average_metrics(
        self, metrics_list: List[ModelPerformanceMetrics]
    ...
    def _detect_drift(self, metrics: ModelPerformanceMetrics) -> List[DriftAlert]:
        ...
        stat_alerts = self.statistical_detector.detect_drift(
            metrics, self.baseline_metrics
        )
        alerts.extend(stat_alerts)

        # Distribution drift detection
        dist_alerts = self.distribution_detector.detect_drift(
            metrics, self.baseline_metrics
        )
    ...
    def get_drift_summary(self) -> Dict[str, Any]:
        return {
            "baseline_timestamp": (
                self.baseline_metrics.timestamp.isoformat()
                if self.baseline_metrics
                else None
            ),
