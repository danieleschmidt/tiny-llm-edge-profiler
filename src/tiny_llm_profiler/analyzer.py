...
class MetricsAnalyzer:
    ...
    def analyze(self, results: ProfileResults) -> Dict[str, Any]:
            ...
            analysis.update(
                {
                    ...
                    "latency_variability": (
                        latency.latency_std_ms / latency.total_latency_ms
                        if latency.total_latency_ms > 0
                        else 0.0
                    ),
                }
            )
        ...
        if results.memory_profile:
            ...
            analysis.update(
                {
                    ...
                    "memory_overhead_kb": memory.peak_memory_kb
                    - memory.baseline_memory_kb,
            ...
            if (
                ...
                and results.latency_profile.tokens_per_second > 0
            ):
                tokens_per_kb = results.latency_profile.tokens_per_second / (
                    memory.memory_usage_kb / 1000
                )
            ...
            analysis.update(
                {
                    ...
                    "power_efficiency": (
                        1000.0 / power.energy_per_token_mj
                        if power.energy_per_token_mj > 0
                        else 0.0
                    ),  # tokens per joule
                }
            )
    ...
    def generate_report(self, results: ProfileResults, output_path: str):
        ...
        if results.latency_profile:
            ...
            fig.add_trace(
                go.Bar(
                    ...
                    y=[
                        latency.first_token_latency_ms,
                        latency.inter_token_latency_ms,
        ...
        fig.update_layout(
            height=800, title_text=f"Performance Analysis: {results.model_name}"
        )
...
class ComparativeAnalyzer:
    ...
    def compare(
        self, comparison_data: List[Tuple[str, str, ProfileResults]]
    ...
    def _calculate_performance_score(self, results: ProfileResults) -> float:
        ...
        if results.memory_profile and results.memory_profile.memory_usage_kb > 0:
            memory_efficiency = (
                1000.0 / results.memory_profile.memory_usage_kb
            )  # Inverse of memory usage
    ...
    def plot_comparison(
        self, results: List[ComparisonResult], metrics: List[str], save_to: str
    ):
        ...
        fig = make_subplots(
            rows=len(metrics), cols=1, subplot_titles=metrics, vertical_spacing=0.1
        )
        ...
        for i, metric in enumerate(metrics, 1):
            ...
            fig.add_trace(
                go.Bar(x=models, y=values, name=metric.title(), showlegend=(i == 1)),
                row=i,
                col=1,
            )
    ...
    def recommend_deployment(
        ...
        for result in self.comparisons:
            ...
            if "max_latency_ms" in constraints and result.profile.latency_profile:
                if (
                    result.profile.latency_profile.total_latency_ms
            ...
            if "max_memory_kb" in constraints and result.profile.memory_profile:
                if (
                    result.profile.memory_profile.peak_memory_kb
                    > constraints["max_memory_kb"]
                ):
                    meets_constraints = False
            ...
            if "max_power_mw" in constraints and result.profile.power_profile:
                if (
                    result.profile.power_profile.active_power_mw
