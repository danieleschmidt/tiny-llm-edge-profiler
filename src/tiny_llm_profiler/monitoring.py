...
class HealthMonitor:
    ...
    def get_alerts(
        self, level: Optional[str] = None, last_n: Optional[int] = None
    ) -> List[Dict[str, Any]]:
...
class ProfilingMonitor:
    ...
    def stop_profiling(self):
        ...
        self.logger.info(
            f"Stopped profiling monitoring: {self.profiling_name} (duration: {duration:.1f}s)"
        )

        self._checkpoint(
            "profiling_completed",
            {"name": self.profiling_name, "duration_seconds": duration},
        )
...
class AlertManager:
    ...
    def console_handler(alert: Alert):
        ...
        print(
            f"[{timestamp}] ALERT [{alert.level.upper()}] {alert.source}: {alert.message}"
        )
