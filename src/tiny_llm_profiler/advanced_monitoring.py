...
class MetricCollector:
    ...
    def get_latest(self, metric_name: str, labels: Optional[Dict[str, str]] = None) -> Optional[Metric]:
...
class AlertManager:
    ...
    def silence_alert(self, alert_id: str, duration_minutes: int = 60):
        ...
        if alert_id in self.active_alerts:
            ...
            self.logger.info(f"Silenced alert {alert_id} for {duration_minutes} minutes")
        ...
        if condition_met:
            ...
            if alert_id not in self.active_alerts:
                ...
                if rule.duration_seconds > 0:
                    ...
                    if not all(self._check_condition(rule, m.value) for m in historical_metrics):
...
class MonitoringSystem:
    ...
    def _setup_alert_handlers(self):
        ...
        def console_handler(alert: Alert):
            ...
            print(f"{emoji} [{alert.severity.value.upper()}] {state_msg}: {alert.description}")
