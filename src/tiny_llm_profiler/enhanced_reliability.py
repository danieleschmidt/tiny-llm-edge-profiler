        ...
        with self._lock:
            ...
            if self.state == CircuitState.OPEN:
                if self._should_attempt_reset():
                    ...
                    logger.info(
                        f"Circuit breaker {func.__name__} moved to HALF_OPEN state"
                    )
                else:
                    if self.config.fallback_function:
                        logger.warning(
                            f"Circuit breaker {func.__name__} OPEN, using fallback"
                        )
    ...
    def _record_success(self) -> None:
        ...
        if self.state == CircuitState.HALF_OPEN:
            ...
            if self.half_open_calls >= self.config.half_open_max_calls:
                ...
                self.stats.state_changes += 1
                logger.info(
                    f"Circuit breaker moved to CLOSED state after successful recovery"
                )
    ...
    def _record_failure(self) -> None:
        ...
        if self.state == CircuitState.CLOSED:
            if self.stats.failed_calls >= self.config.failure_threshold:
                ...
                self.stats.state_changes += 1
                logger.warning(
                    f"Circuit breaker moved to OPEN state after {self.stats.failed_calls} failures"
                )
        elif self.state == CircuitState.HALF_OPEN:
            self.state = CircuitState.OPEN
            self.stats.state_changes += 1
            logger.warning(
                f"Circuit breaker moved back to OPEN state during recovery attempt"
            )
...
class BulkheadPattern:
    ...
    async def _execute_with_bulkhead(self, func: Callable, *args, **kwargs):
        ...
        try:
            ...
            await asyncio.wait_for(
                self.semaphore.acquire(), timeout=self.config.timeout_seconds
            )
            ...
            raise Exception(
                f"Bulkhead timeout: {func.__name__} took longer than {self.config.timeout_seconds}s"
            )
    ...
    def get_stats(self) -> Dict[str, Any]:
        return {
            ...
            "success_rate": (
                self.total_calls - self.rejected_calls - self.timeout_calls
            )
...
class ChaosEngineer:
    ...
    def should_inject_chaos(
        self, service_name: str, experiment_type: ChaosExperimentType
    ) -> bool:
        for exp_id, experiment in self.experiments.items():
            if (
                experiment.enabled
                and experiment.target_service == service_name
                and experiment.experiment_type == experiment_type
                and random.random() < experiment.probability
            ):
                return True
        return False

    async def inject_latency(
        self, service_name: str, base_latency: float = 0.1
    ) -> None:
        if self.should_inject_chaos(
            service_name, ChaosExperimentType.LATENCY_INJECTION
        ):
            ...
            experiment = next(
                (
                    ...
                    if exp.target_service == service_name
                    and exp.experiment_type == ChaosExperimentType.LATENCY_INJECTION
                ),
                None,
            )
            ...
            if experiment:
                ...
                logger.warning(
                    f"Chaos Engineering: Injecting {additional_delay:.3f}s latency to {service_name}"
                )
                await asyncio.sleep(total_delay)

                self._record_experiment_execution(
                    experiment.experiment_id,
                    {"delay_injected": additional_delay, "total_delay": total_delay},
                )

    def inject_error(
        self, service_name: str, error_message: str = "Chaos-induced error"
    ) -> None:
        if self.should_inject_chaos(service_name, ChaosExperimentType.ERROR_INJECTION):
            experiment = next(
                (
                    ...
                    for exp in self.experiments.values()
                    if exp.target_service == service_name
                    and exp.experiment_type == ChaosExperimentType.ERROR_INJECTION
                ),
                None,
            )
            ...
            if experiment:
                ...
                logger.warning(
                    f"Chaos Engineering: Injecting {error_type} error to {service_name}"
                )
                ...
                if error_type == "timeout":
                    ...
                    raise ConnectionError(
                        f"Chaos-induced connection error in {service_name}"
                    )
                else:
                    raise Exception(
                        f"Chaos-induced error in {service_name}: {error_message}"
                    )

    def _record_experiment_execution(
        self, experiment_id: str, details: Dict[str, Any]
    ) -> None:
        execution_record = {
    ...
    def _retry_with_backoff(self, func: Callable, *args, **kwargs):
        ...
        for attempt in range(self.max_attempts):
            try:
                ...
                logger.warning(
                    f"Retry attempt {attempt + 1} failed for {func.__name__}, retrying in {delay:.2f}s: {str(e)}"
                )
                time.sleep(delay)
...
class ReliabilityManager:
    ...
    def protect_function(
        ...
        chaos_service_name: Optional[str] = None,
    ) -> Callable:
