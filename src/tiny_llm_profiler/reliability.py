...
class RetryableError(TinyLLMProfilerError):
    ...
    def __init__(
        ...
        super().__init__(
            message, "RETRYABLE_ERROR", {"original_error": str(original_error)}
        )
...
class CircuitOpenError(TinyLLMProfilerError):
    ...
    def __init__(
        self, circuit_name: str, failure_count: int, last_failure_time: datetime
    ):
...
class RetryMechanism:
    ...
    def _execute_with_retry(self, func: Callable, args: tuple, kwargs: dict) -> Any:
        ...
        for attempt in range(1, self.config.max_attempts + 1):
            try:
                self.logger.debug(
                    f"Attempting {func.__name__}, attempt {attempt}/{self.config.max_attempts}"
                )
...
class CircuitBreaker:
    ...
    def _on_failure(self, exception: Exception):
        ...
        with self._lock:
            ...
            cutoff_time = self.last_failure_time - timedelta(
                seconds=self.config.monitor_window_seconds
            )
            self.failure_history = [
                ts for ts in self.failure_history if ts > cutoff_time
            ]

            self.logger.warning(
                f"Circuit {self.name} failure #{self.failure_count}: {exception}"
            )
    ...
    def _should_open_circuit(self) -> bool:
        ...
        if len(self.failure_history) >= 2:  # Need at least 2 data points
            ...
            failure_rate = (
                recent_failures / total_operations if total_operations > 0 else 0
            )
...
class TimeoutManager:
    ...
    def timeout(
        self, seconds: Optional[float] = None, operation_name: str = "operation"
    ):
        ...
        try:
            self.logger.debug(
                f"Starting {operation_name} with {timeout_duration}s timeout"
            )
            yield
...
class ResourceManager:
    ...
    def register_resource(
        ...
        with self._lock:
            ...
            self.logger.debug(
                f"Registered {category} resource: {type(resource).__name__}"
            )
        ...
        with self._lock:
            ...
            for category, resources in self.resource_registry.items():
                ...
                if stats["by_category"][category]["oldest"]:
                    stats["by_category"][category]["oldest"] = stats["by_category"][
                        category
                    ]["oldest"].isoformat()
                if stats["by_category"][category]["newest"]:
                    stats["by_category"][category]["newest"] = stats["by_category"][
                        category
                    ]["newest"].isoformat()
...
class GracefulDegradation:
    ...
    def register_fallback(
        self, operation_name: str, fallback_func: Callable, priority: int = 0
    ):
        ...
        self.fallback_functions[operation_name].append(
            {"func": fallback_func, "priority": priority}
        )
        ...
        self.logger.debug(
            f"Registered fallback for {operation_name} with priority {priority}"
        )
        ...
        def decorator(func: F) -> F:
            ...
            def wrapper(*args, **kwargs):
                return self._execute_with_fallback(
                    operation_name, func, args, kwargs, fallback_funcs
                )
    ...
    def _execute_with_fallback(
        ...
        try:
            ...
            self.logger.warning(
                f"Primary function failed for {operation_name}: {primary_error}"
            )
            ...
            if operation_name in self.fallback_functions:
                all_fallbacks.extend(
                    [fb["func"] for fb in self.fallback_functions[operation_name]]
                )
            ...
            for i, fallback_func in enumerate(all_fallbacks):
                try:
                    ...
                    self.logger.warning(
                        f"Fallback #{i+1} failed for {operation_name}: {fallback_error}"
                    )
                    continue
...
def with_fallback(*fallback_funcs: Callable) -> Callable:
    ...
    def decorator(func: F) -> F:
        ...
        return reliability_manager.graceful_degradation.with_fallback(
            operation_name, *fallback_funcs
        )(func)
...
def managed_resource(
    resource: Any, category: str = "general", cleanup_func: Optional[Callable] = None
):
    """Context manager for automatic resource management."""
    reliability_manager.resource_manager.register_resource(
        resource, category, cleanup_func
    )
    try:
        yield resource
    finally:
