...
from .pipeline_guard import (
    SelfHealingPipelineGuard, get_pipeline_guard, PipelineStage, HealthStatus
)
from .model_drift_detector import (
    ModelDriftMonitor, get_drift_monitor, DriftType, DriftSeverity
)
from .infrastructure_sentinel import (
    InfrastructureSentinel, get_infrastructure_sentinel, ResourceType, AlertLevel
)
...
class UnifiedGuardSystem:
    ...
    def _calculate_overall_status(
        ...
        if (not pipeline_health.get("running", False) or
            infrastructure_status.get("overall_status") == "failed"):
            return SystemStatus.FAILED
        
        # Check for critical states
        if (pipeline_health.get("active_failures", 0) > 0 or
            drift_status.get("active_alerts_by_severity", {}).get("critical", 0) > 0 or
            infrastructure_status.get("overall_status") == "critical"):
            return SystemStatus.CRITICAL
        
        # Check for warning states
        if (drift_status.get("active_alerts_count", 0) > 0 or
            infrastructure_status.get("overall_status") == "degraded"):
    ...
    async def _analyze_system_health(self, metrics: SystemMetrics) -> List[SystemAlert]:
        ...
        infra_alerts = self._analyze_infrastructure_status(metrics.infrastructure_status)
    ...
    def _analyze_pipeline_health(self, pipeline_health: Dict[str, Any]) -> List[SystemAlert]:
        alerts = []
        
        # Check for active pipeline failures
        active_failures = pipeline_health.get("active_failures", 0)
        if active_failures > 0:
            priority = AlertPriority.HIGH if active_failures > 2 else AlertPriority.MEDIUM
    ...
    def _analyze_drift_status(self, drift_status: Dict[str, Any]) -> List[SystemAlert]:
        ...
        critical_alerts = drift_status.get("active_alerts_by_severity", {}).get("critical", 0)
    ...
    def _analyze_infrastructure_status(self, infra_status: Dict[str, Any]) -> List[SystemAlert]:
        alerts = []
    ...
    def _analyze_correlations(self, metrics: SystemMetrics) -> List[SystemAlert]:
        ...
        infra_critical = metrics.infrastructure_status.get("overall_status") == "critical"
    ...
    async def _handle_emergency_alert(self, alert: SystemAlert) -> None:
        ...
        if alert.alert_type == "cascading_failure":
            ...
            logger.critical("Infrastructure failure - check system resources immediately")
    ...
    async def _check_system_recovery(self) -> None:
        ...
        critical_alerts = [
            ...
            if not alert.resolved and alert.priority in [AlertPriority.CRITICAL, AlertPriority.EMERGENCY]
        ]
        ...
        if not critical_alerts:
            ...
            recent_alerts = [
                alert for alert in self.system_alerts
                if not alert.resolved and (datetime.now() - alert.timestamp).seconds < 300
            ]
    ...
    def _calculate_current_status(self) -> SystemStatus:
        ...
        if any(alert.priority == AlertPriority.EMERGENCY for alert in active_alerts):
            ...
            return SystemStatus.CRITICAL
        elif any(alert.priority in [AlertPriority.HIGH, AlertPriority.MEDIUM] for alert in active_alerts):
            return SystemStatus.WARNING
        else:
            return SystemStatus.HEALTHY
