    ...
    def _monitor_loop(self):
        ...
        while self._running:
            try:
                ...
                self._metrics.memory_usage_mb = psutil.virtual_memory().used / (
                    1024 * 1024
                )
...
class HealthMonitor:
                ...
                try:
                    ...
                    if result:
                        ...
                        if check.critical and check.consecutive_failures >= 3:
                            alert_msg = (
                                f"CRITICAL: Health check '{check.name}' failed 3+ times"
                            )
    ...
    def _check_alert_conditions(self, metrics: HealthMetrics):
        ...
        if metrics.avg_response_time_ms > 5000:  # 5 seconds
            self.alerts.append(
                f"High response time: {metrics.avg_response_time_ms:.0f}ms"
            )
