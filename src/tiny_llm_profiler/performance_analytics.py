...
class StatisticalAnalyzer:
    ...
    def detect_outliers(values: List[float], method: str = "iqr") -> List[int]:
        ...
        if method == "iqr":
            ...
            outlier_indices = [
                i for i, v in enumerate(values) if abs(v - mean) > 3 * std
            ]
...
class RegressionDetector:
    ...
    def __init__(
        ...
        self.regression_history: Dict[str, List[RegressionDetectionResult]] = (
            defaultdict(list)
        )
    ...
    def detect_regression(
    ...
    ) -> Optional[RegressionDetectionResult]:
        ...
        if len(history) < self.min_samples:
            return RegressionDetectionResult(
                metric_name=metric_name, regression_detected=False
            )
        ...
        if lower_is_better:
            ...
            degradation_ratio = (
                (recent_mean - baseline_mean) / baseline_mean
                if baseline_mean > 0
                else 0
            )
        else:
            ...
            degradation_ratio = (
                (baseline_mean - recent_mean) / baseline_mean
                if baseline_mean > 0
                else 0
            )
        ...
        evidence = self._generate_evidence(
            baseline_values, recent_values, degradation_percentage
        )
        ...
        if len(self.regression_history[metric_name]) > 50:
            self.regression_history[metric_name] = self.regression_history[metric_name][
                -25:
            ]
    ...
    def _classify_regression_severity(
        self, degradation_percentage: float
    ) -> RegressionSeverity:
    ...
    def _generate_evidence(
        self, baseline: List[float], recent: List[float], degradation_percentage: float
    ) -> List[str]:
        ...
        if recent_std > baseline_std * 1.5:
            evidence.append(
                f"Increased variability detected (std: {baseline_std:.3f} -> {recent_std:.3f})"
            )

        # Check for trend
        timestamps = list(range(len(recent)))
        trend = StatisticalAnalyzer.calculate_trend(
            recent, [datetime.now() - timedelta(seconds=i) for i in timestamps]
        )

        if abs(trend["slope"]) > 0.001 and trend["r_squared"] > 0.3:
            trend_direction = "increasing" if trend["slope"] > 0 else "decreasing"
            evidence.append(
                f"Strong {trend_direction} trend detected (R²={trend['r_squared']:.3f})"
            )
        ...
        for metric_name, results in self.regression_history.items():
            for result in results:
                if result.regression_detected:
                    ...
                    if result.detection_timestamp > datetime.now() - timedelta(
                        hours=24
                    ):
...
class AnomalyDetector:
    ...
    def __init__(
        ...
        self.anomaly_history: Dict[str, List[AnomalyDetectionResult]] = defaultdict(
            list
        )
    ...
    def detect_anomaly(
        ...
        anomaly_type = (
            "high_outlier" if current_value > stats["mean"] else "low_outlier"
        )
...
class OptimizationEngine:
    ...
    def generate_recommendations(
        ...
        for rule in self.recommendation_rules:
            try:
                rule_recommendations = rule(
                    metrics, regression_results, anomaly_results
                )
                ...
                if result.severity in [
                    ...
                    recommendations.append(
                        OptimizationRecommendation(
                            ...
                            expected_improvement=result.degradation_percentage
                            * 0.8,  # Assume 80% recovery
                ...
                recommendations.append(
                    OptimizationRecommendation(
                        ...
                        priority=(
                            "high"
                            if result.severity
                            in [RegressionSeverity.MAJOR, RegressionSeverity.CRITICAL]
                            else "medium"
                        ),
                    )
                )
...
class PerformanceReporter:
    ...
    def generate_performance_report(
        ...
        active_regressions = sum(
            1 for r in regression_results.values() if r.regression_detected
        )
        total_anomalies = sum(
            len(anomaly_list) for anomaly_list in anomaly_results.values()
        )
        ...
        for metric_name, metric_list in metrics.items():
            if metric_list:
                ...
                metric_summaries[metric_name] = (
                    StatisticalAnalyzer.calculate_percentiles(values)
                )
        ...
        report = {
            ...
            "recent_anomalies": {
                name: [
                    asdict(anomaly) for anomaly in anomaly_list[-10:]
                ]  # Last 10 anomalies
                for name, anomaly_list in anomaly_results.items()
                if anomaly_list
            },
            "optimization_recommendations": [
                r.to_dict() for r in recommendations[:10]
            ],  # Top 10
...
class PerformanceAnalyticsEngine:
                ...
                for metric_name in metric_names:
                    try:
                        ...
                        if result:
                            ...
                            if result.regression_detected:
                                logger.warning(
                                    f"Regression detected: {result.to_alert_message()}"
                                )

                    except Exception as e:
                        logger.error(
                            f"Regression analysis error for {metric_name}: {e}"
                        )
                        ...
                        for metric in metric_list[-5:]:  # Check last 5 metrics
                            try:
                                ...
                                if result and result.anomaly_detected:
                                    ...
                                    self.latest_anomaly_results[metric_name].append(
                                        result
                                    )
                                ...
                                logger.error(
                                    f"Anomaly analysis error for {metric_name}: {e}"
                                )
    ...
    def get_analytics_stats(self) -> Dict[str, Any]:
        ...
        with self.metrics_lock:
            total_metrics = sum(
                len(metric_list) for metric_list in self.metrics.values()
            )
        ...
        stats = {
            ...
            "analysis_results": {
                "active_regressions": sum(
                    1
                    for r in self.latest_regression_results.values()
                    if r.regression_detected
                ),
                "recent_anomalies": sum(
                    len(anomaly_list)
                    for anomaly_list in self.latest_anomaly_results.values()
                ),
        ...
        if self.enable_regression_detection:
            stats["regression_detector"] = (
                self.regression_detector.get_regression_summary()
            )
