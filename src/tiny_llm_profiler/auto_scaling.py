...
class AutoScaler:
    ...
    def add_scaling_callback(
        self, callback: Callable[[ScalingDecision, int, int], None]
    ):
    ...
    def make_scaling_decision(self) -> Tuple[ScalingDecision, int]:
        ...
        # Scale up conditions
        scale_up_needed = (
            avg_queue_length > self.config.scale_up_queue_threshold
            or avg_cpu_util > self.config.cpu_scale_up_threshold
            or avg_memory_util > self.config.memory_scale_up_threshold
            or avg_response_time > self.config.response_time_threshold
            or avg_error_rate > self.config.error_rate_threshold
        )
        ...
        scale_down_needed = (
            avg_queue_length < self.config.scale_down_queue_threshold
            and avg_cpu_util < self.config.cpu_scale_down_threshold
            and avg_memory_util < 50.0  # Conservative memory threshold for scale down
            and avg_response_time < self.config.response_time_threshold * 0.5
            and avg_error_rate < self.config.error_rate_threshold * 0.1
        )
        ...
        if self.enable_predictive_scaling:
            ...
            if predicted_decision != ScalingDecision.MAINTAIN:
                if predicted_decision == ScalingDecision.SCALE_UP:
                    ...
                    target_workers = max(
                        self.current_workers - 1, self.config.min_workers
                    )
                    return predicted_decision, target_workers

        return ScalingDecision.MAINTAIN, self.current_workers

    def execute_scaling_decision(
        self, decision: ScalingDecision, target_workers: int
    ) -> bool:
        """
        ...
        self.scaling_history = [
            (timestamp, dec, workers)
            for timestamp, dec, workers in self.scaling_history
    ...
    def get_scaling_stats(self) -> Dict[str, Any]:
        ...
        with self.metrics_lock:
            ...
            recent_scaling = [
                timestamp
                for timestamp, _, _ in self.scaling_history
            ...
            return {
                ...
                "scaling_activity": {
                    ...
                    "last_scaling": (
                        self.last_scaling_decision.isoformat()
    ...
    def _get_recent_metrics(self) -> List[ScalingMetrics]:
        ...
        cutoff_time = datetime.now() - timedelta(
            seconds=self.config.evaluation_window_seconds
        )
    ...
    def _needs_emergency_scaling(self, metrics: ScalingMetrics) -> bool:
        ...
        return (
            metrics.queue_length > self.config.emergency_queue_threshold
            or metrics.cpu_utilization > self.config.emergency_cpu_threshold
            or metrics.memory_utilization > 95.0
            or metrics.error_rate > 0.5  # 50% error rate
        )
    ...
    def _predict_scaling_need(self) -> ScalingDecision:
        ...
        response_time_trend = self._calculate_trend(
            [m.avg_response_time for m in recent]
        )
        ...
        if (
            predicted_queue > self.config.scale_up_queue_threshold
            or predicted_cpu > self.config.cpu_scale_up_threshold
            or predicted_response > self.config.response_time_threshold
        ):
            return ScalingDecision.SCALE_UP

        if (
            predicted_queue < self.config.scale_down_queue_threshold
            and predicted_cpu < self.config.cpu_scale_down_threshold
            and predicted_response < self.config.response_time_threshold * 0.5
        ):
...
class LoadBalancer:
    ...
    def register_worker(
        self, worker_id: str, capabilities: Optional[Dict[str, Any]] = None
    ):
    ...
    def remove_worker(self, worker_id: str):
        ...
        with self.lock:
            if worker_id in self.worker_stats:
                ...
                self.logger.info(
                    f"Removed worker: {worker_id} ({len(active_tasks)} tasks reassigned)"
                )
    ...
    def assign_task(self, task: ProfilingTask) -> Optional[str]:
        ...
        with self.lock:
            ...
            strategy_func = self.strategies.get(
                self.current_strategy, self._round_robin_selection
            )
            ...
            if selected_worker:
                ...
                self.logger.debug(
                    f"Assigned task {task.task_id} to worker {selected_worker}"
                )
                ...
                if worker_id in self.worker_stats:
                    ...
                    if stats["completed_tasks"] > 0:
                        stats["avg_response_time"] = (
                            stats["total_response_time"] / stats["completed_tasks"]
                        )
    ...
    def get_load_balancing_stats(self) -> Dict[str, Any]:
        ...
        with self.lock:
            total_active = sum(
                stats["active_tasks"] for stats in self.worker_stats.values()
            )
            total_completed = sum(
                stats["completed_tasks"] for stats in self.worker_stats.values()
            )
            total_errors = sum(
                stats["error_count"] for stats in self.worker_stats.values()
            )
    ...
    def _calculate_selection_score(
        self, worker_id: str, task: ProfilingTask, stats: Dict[str, Any]
    ) -> float:
