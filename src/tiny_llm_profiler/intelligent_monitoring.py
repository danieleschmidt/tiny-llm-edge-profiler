...
class AdaptiveThresholdManager:
    ...
    def _update_adaptive_threshold(self, metric_name: str, new_value: float) -> None:
        ...
        recent_values = [
            entry["value"] for entry in list(history)[-50:]
        ]  # Last 50 values
        ...
        if std_value > 0:
            ...
            threshold.warning_deviation = max(
                0.1, threshold.warning_deviation * variance_factor
            )
            threshold.critical_deviation = max(
                0.2, threshold.critical_deviation * variance_factor
            )
    ...
    def get_threshold_status(self, metric_name: str) -> Dict[str, Any]:
        ...
        return {
            ...
            * (1 + threshold.warning_deviation),
            "critical_threshold": threshold.baseline_value
            * (1 + threshold.critical_deviation),
...
class AnomalyDetector:
    ...
    def train_model(self, metric_name: str, training_data: List[float]) -> None:
        ...
        if len(training_data) < 30:
            self.logger.warning(
                f"Insufficient training data for {metric_name}: {len(training_data)} samples"
            )
            return
    ...
    def detect_anomaly(
        ...
        z_score = (
            abs(current_value - model["mean"]) / model["std"] if model["std"] > 0 else 0
        )

        # Percentile-based detection
        percentiles = model["percentiles"]
        is_outlier_percentile = (
            current_value < percentiles[0]  # Below 1st percentile
            or current_value > percentiles[6]  # Above 99th percentile
        )
        ...
        if is_anomaly:
            anomaly_info = {
                ...
                "severity": self._calculate_anomaly_severity(
                    z_score, is_outlier_percentile
                ),
                "confidence": self._calculate_confidence(
                    z_score, percentiles, current_value
                ),
                "timestamp": time.time(),
                "context": context or {},
            }
    ...
    def _calculate_anomaly_severity(
        self, z_score: float, is_outlier_percentile: bool
    ) -> AlertSeverity:
    ...
    def _calculate_confidence(
        self, z_score: float, percentiles: np.ndarray, current_value: float
    ) -> float:
...
class CorrelationAnalyzer:
        ...
        for i, metric1 in enumerate(metric_names):
            for j, metric2 in enumerate(metric_names[i + 1 :], i + 1):
                ...
                if abs(correlation) > self.correlation_threshold:
                    ...
                    if not any(
                        p["metric1"] == metric1 and p["metric2"] == metric2
    ...
    def analyze_alert_correlations(
        self, triggered_metric: str, current_metrics: Dict[str, float]
    ) -> List[str]:
...
class IntelligentMonitoringSystem:
    ...
    async def start_intelligent_monitoring(
        self, monitoring_interval_seconds: float = 5.0
    ) -> None:
    ...
    async def _check_threshold_violations(
        ...
        for metric_name, value in current_metrics.items():
            violation = self.threshold_manager.check_threshold_violation(
                metric_name, value
            )
            ...
            if violation:
                ...
                await self._generate_threshold_alert(
                    metric_name, value, severity, deviation
                )
    ...
    async def _generate_anomaly_alert(self, anomaly_info: Dict[str, Any]) -> None:
        ...
        alert = IntelligentAlert(
            ...
            expected_value=anomaly_info["expected_range"][
                0
            ],  # Use lower bound as expected
    ...
    def _generate_threshold_recommendations(
        self, metric_name: str, deviation: float
    ) -> List[str]:
    ...
    def _generate_anomaly_recommendations(
        self, anomaly_info: Dict[str, Any]
    ) -> List[str]:
    ...
    async def _evaluate_alert_rule(
        self, rule: AlertRule, current_metrics: Dict[str, float]
    ) -> bool:
    ...
    def train_anomaly_detector(
        self, metric_name: str, training_data: List[float]
    ) -> None:
    ...
    async def get_monitoring_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "threshold_manager": {
                "adaptive_thresholds": len(
                    [
                        t
                        for t in self.threshold_manager.thresholds.values()
                        if t.adaptive
                    ]
                ),
                "metrics_with_baseline": len(
                    self.threshold_manager.baseline_statistics
                ),
            },
            "anomaly_detector": {
                ...
                "total_anomalies_detected": sum(
                    len(history)
                    for history in self.anomaly_detector.anomaly_history.values()
                ),
            },
            "correlation_analyzer": {
                "correlation_patterns": len(
                    self.correlation_analyzer.correlation_patterns
                ),
